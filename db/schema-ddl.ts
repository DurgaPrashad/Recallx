/**
 * The initial schema DDL, embedded as plain strings (mirrors
 * db/migrations/0000_minor_tarantula.sql). Used to bootstrap the in-memory
 * fallback database at runtime (see db/index.ts) — that path can't rely on
 * reading the .sql migration file off disk inside a serverless bundle, so
 * the statements live in code instead, guaranteed to ship with the build.
 *
 * If you change db/schema.ts, run `npm run db:generate` as usual for the
 * real migration file, and mirror the new statements here.
 */
export const CREATE_TABLES_SQL: string[] = [
  ...
  "CREATE UNIQUE INDEX `incidents_key_unique` ON `incidents` (`key`)",
  ...
  "CREATE UNIQUE INDEX `resolutions_incident_id_unique` ON `resolutions` (`incident_id`)",
